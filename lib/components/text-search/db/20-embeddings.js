...
CREATE VIEW text_search_document_vector AS
    SELECT
...
CREATE RULE on_delete AS ON DELETE TO text_search_document_vector DO INSTEAD DELETE FROM text_search_document WHERE id = OLD.id;
...
BEGIN
    ...
    IF ( SELECT unique_document FROM text_search_storage WHERE id = _storage_id ) THEN
        SELECT id FROM text_search_document WHERE embedding_id = _embedding_id INTO _document_id;
