...
CREATE SEQUENCE text_search_embedding_document_id_seq AS int8 MAXVALUE ${ Number.MAX_SAFE_INTEGER };

CREATE TABLE text_search_embedding_document (
    id int53 PRIMARY KEY DEFAULT nextval( 'text_search_embedding_document_id_seq' ),
...
ALTER SEQUENCE text_search_embedding_document_id_seq OWNED BY text_search_embedding_document.id;

CREATE INDEX text_search_embedding_document_storage_id_embedding_id_idx ON text_search_embedding_document ( storage_id, embedding_id );

CREATE FUNCTION text_search_embedding_document_after_delete_trigger () RETURNS trigger AS $$
BEGIN
    IF ( NOT EXISTS ( SELECT FROM text_search_embedding_document WHERE storage_id = OLD.storage_id AND embedding_id = OLD.embedding_id ) ) THEN
...
CREATE TRIGGER text_search_embedding_document_after_delete AFTER DELETE ON text_search_embedding_document FOR EACH ROW EXECUTE FUNCTION text_search_embedding_document_after_delete_trigger();

CREATE VIEW text_search_document AS
    SELECT
        text_search_embedding_document.id,
        text_search_embedding_document.storage_id,
        text_search_embedding_document.created,
        text_search_embedding.content,
        text_search_embedding.vector
    FROM
        text_search_embedding_document,
        text_search_embedding
    WHERE
        text_search_embedding_document.storage_id = text_search_embedding.storage_id
        AND text_search_embedding_document.embedding_id = text_search_embedding.embedding_id
;

CREATE RULE on_delete AS ON DELETE TO text_search_document DO INSTEAD DELETE FROM text_search_embedding_document WHERE id = OLD.id;
...
CREATE FUNCTION get_text_search_document_vector ( _document_id int53 ) RETURNS vector STABLE AS $$
BEGIN
...
CREATE FUNCTION create_text_search_document ( _storage_id int53, _content text, _vector vector ) RETURNS int53 AS $$
DECLARE
    _document_id int53;
    _embedding_id int8;
BEGIN
    ...
    IF ( SELECT unique_document FROM text_search_storage WHERE id = _storage_id ) THEN
        SELECT id FROM text_search_embedding_document WHERE storage_id = _storage_id AND embedding_id = _embedding_id INTO _document_id;

        IF _document_id THEN
            RETURN _document_id;
        END IF;
    END IF;
