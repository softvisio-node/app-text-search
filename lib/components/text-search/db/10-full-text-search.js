...
CREATE FUNCTION get_text_search_tsvector ( content text ) RETURNS tsvector IMMUTABLE AS $$
BEGIN
    RETURN get_text_search_tsvector ( content, NULL );
END;
$$ LANGUAGE plpgsql;
...
CREATE FUNCTION get_text_search_tsquery ( content text ) RETURNS tsquery IMMUTABLE AS $$
BEGIN
    RETURN get_text_search_tsquery ( content, NULL );
END;
$$ LANGUAGE plpgsql;
