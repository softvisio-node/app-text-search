...
export default class {
    ...
    async #createDocument ( storage, hash, { content, dbh } = {} ) {
        ...
        res = await this.#getEmbeddings( storage.model, content, storage.documentType );
        if ( !res.ok ) return res;
    ...
    async #getEmbeddings ( model, text, documentType ) {
        return this.app.services.get( "ai" ).getEmbeddings( model, text, {
            documentType,
        } );
    }
