...
export default class {
    ...
    async createDocument ( storageId, text, { dbh } = {} ) {
        const hash = crypto.createHash( "MD5" ).update( text ).digest( "base64url" );

        var res;

        res = await this.#createDocument( storageId, hash, { dbh } );
        ...
        res = await this.#createDocument( storageId, hash, { text, dbh } );
    ...
    async #createDocument ( storageId, hash, { text, dbh } = {} ) {
        dbh ||= this.dbh;

        var res;

        res = await dbh.selectRow( SQL.createDocument, [ storageId, hash, null ] );
        ...
        if ( !storage ) {
            ...
            if ( !res.data ) return result( [ 400, "Text search storage not found" ] );

            storage[ storageId ] = res.data;
        }

        res = await this.#getEmbedding( text, storage.model, storage.document_type );
        if ( !res.ok ) return res;

        const vector = res.data;

        return dbh.selectRow( SQL.createDocument, [ storageId, hash, vector ] );
    }
