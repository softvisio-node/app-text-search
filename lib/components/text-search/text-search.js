...
export default class {
    ...
    async init () {
        ...
        const docsId = res.data.id;

        res = await this.createStorage( "Xenova/all-MiniLM-L6-v2", "RETRIEVAL_QUERY", { "storeContent": true } );
        const queryId = res.data.id;

        res = await this.createDocument( docsId, "продам новый шкаф" );
        res = await this.createDocument( docsId, "нупите у меня шкаф шкаф" );
        res = await this.createDocument( docsId, "небо сегодня голубое" );
