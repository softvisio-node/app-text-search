...
export default class {
    ...
    async init () {
        ...
        // init models
        res = await this.dbh.do( sql`INSERT INTO text_search_model`.VALUES( Object.keys( AiApi.models )
            .filter( name => AiApi.models[ name ].embeddingSize )
            .map( name => {
                return {
                    name,
                    "vector_dimensions": AiApi.models[ name ].embeddingSize,
                };
    ...
    async getStorageEmbeddingSize ( storageId ) {
        const storage = await this.#getStorage( storageId );

        return AiApi.models[ storage?.model ]?.embeddingSize;
    }
